...
import (
	...
	"github.com/phen/golang-bookstore/config"
	"github.com/phen/golang-bookstore/models"
...
func login(c echo.Context) error {
	...
	if username != "phen" || password != "pepen" {
...
func main() {
	...
	r.GET("/show/:email", func(c echo.Context) error {
		user, err := models.GetOneByEmail(c.Param("email")) // method get by email
		response := new(Response)
	...
	r.DELETE("/delete/:email", func(c echo.Context) error {
		user, _ := models.GetOneByEmail(c.Param("email")) // method get by email
		response := new(Response)
